...
El usuario accede al frontend. El frontend consume endpoints `/api`. Nginx redirige esas solicitudes al backend. El backend procesa el mensaje, consulta el historial, genera una respuesta con IA o modo de respaldo y guarda mensajes en PostgreSQL.
...
5. El backend guarda el mensaje del usuario.
6. El backend detecta una categoria.
7. El backend genera una respuesta con OpenAI si existe API key; si no, usa modo de respaldo local.
8. El backend guarda la respuesta.
9. El frontend actualiza el historial.
...
- `frontend`: Nginx con archivos estaticos.
- `backend`: aplicacion FastAPI.
- `db`: PostgreSQL.

Docker Compose define dependencias, variables, red interna, volumen de base de datos y puertos publicados.
...
- `messages`: guarda mensajes de usuario y asistente.

El volumen Docker `postgres_data` conserva la informacion incluso si se recrean los contenedores.
...
- Documentar los componentes criticos: VPS, base de datos, archivo `.env`, imagenes Docker y repositorio GitHub.
